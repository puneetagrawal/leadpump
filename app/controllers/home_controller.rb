...
#msg = {"msg"=>"successfull"}
  ...
  def print_waiver_details
    date = params[:data]
    name = params[:name]
    company = current_user.fetchCompany
    @waiver = FrontDeskDesc.where(:user_id=>company)
    @pf = WickedPdf.new.pdf_from_string(render_to_string('common_templates/campaign_temps/_waiver_print.html.erb', :layout=>false))
    respond_to do |format|
      format.pdf do
        send_data @pf, filename: "pass.pdf", type: 'application/pdf', disposition: 'inline'
      end
